...
├── 09_chapter_Loops/
│   ├── 52_Loop.js                            # manual console.log sequence (anti-pattern)
│   ├── 53_For_Loop.js                        # basic for loop — init, condition, increment
│   ├── 54_Increment.js                       # pre-increment (++a) vs post-increment (a++)
│   ├── 55_For_Loops.js                       # for loop range variants (<= vs <)
│   ├── 56_For_Loops2.js                      # for loop with if/else inside body
│   ├── 57_While.js                           # for → while loop conversion comparison
│   ├── 58_While.js                           # while loop — retry/attempts pattern
│   ├── 59_Modie.js                           # while loop — fixed-iteration message
│   ├── 60_While_Vs_For.js                    # while(true) + break idiom
...
**Concept:** Loops execute a block of code repeatedly while a condition holds true. This chapter covers the three loop families in JS — `for`, `while`, and `do-while` — plus nested loops and the pre/post increment distinction that governs loop counter behavior.

**Why:** Every automation framework runs loops: retrying failed assertions, iterating over test data, or spinning up multiple browser contexts. Knowing when to use `for` (known count), `while` (unknown count), or `do-while` (at-least-once) avoids both infinite loops and missed iterations.

**Q&A — why use this?**
- **Q: What's the difference between `for`, `while`, and `do-while`?** A: `for` is the tightest form when you know the iteration count (init, condition, increment all in one line). `while` checks before each run — good for retry loops with unknown iteration counts. `do-while` always runs the body at least once before checking the condition.
- **Q: Why does `62_DoWhile_vs_While.js` exist?** A: To demonstrate the critical difference — `while (a < 10)` with `a = 10` runs zero times; `do { ... } while (a < 10)` with `a = 10` runs exactly once because the condition is checked *after* the body.
- **Q: What's the nested for loop pattern?** A: An outer loop runs the inner loop fully each time, producing a combinatorial grid — `63_NestedFor_lOOP.js` logs all 9 `(i, j)` pairs for a 3×3 matrix.
- **Q: What's the infinite loop risk?** A: `while (true)` without a `break` never exits — `60_While_Vs_For.js` shows the safe pattern with an exit condition inside the body.

```mermaid
flowchart TD
    L{"Which loop?"} --> Fixed{"Known iteration count?"}
    Fixed -->|Yes| For["for (init; condition; increment) — cleanest"]
    Fixed -->|No| CheckFirst{"Body must run at least once?"}
    CheckFirst -->|No| While["while (condition) — check-first"]
    CheckFirst -->|Yes| DoWhile["do { ... } while (condition) — guaranteed-first"]
```

```js
// for loop — known iteration count (13_Practice_For_Loop.js)
for (let i = 1; i <= 10; i++) {
    console.log(i);
}

// while loop — retry/attempts pattern (58_While.js)
let attempts = 0;
while (attempts < 3) {
...
// do-while — always runs body at least once (62_DoWhile_vs_While.js)
let a = 10;
do {
    console.log(a);
    a++;
} while (a < 10);
// Prints 10 — runs once even though condition is false

// Nested for loop — matrix of all (i, j) pairs (63_NestedFor_lOOP.js)
for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
        console.log(i, j);
    }
...
| Loop | Checks condition | Zero iterations safe? | Use case |
|------|:----------------:|:---------------------:|----------|
| `for (init; cond; inc)` | Before each iteration | Yes | Known count |
| `while (cond)` | Before each iteration | Yes | Unknown count / retries |
| `do { ... } while (cond)` | After each iteration | No — runs at least once | Guaranteed-first execution |
